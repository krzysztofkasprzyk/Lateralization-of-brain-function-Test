(function () {
  ...
  const CSV_FIELDS = [
    "participant_ID", "phase", "trial_number", "trial_id", "block_number", "block_type", "trial_type", "word_side", "LVF_stimulus", "RVF_stimulus",
  ...
  const XLSX_COLUMNS = [
    { key: "participant_ID", label: "ID uczestnika" },
    { key: "phase", label: "Faza badania" },
    { key: "trial_number", label: "Numer kolejny próby" },
    { key: "trial_id", label: "ID próby" },
    { key: "block_number", label: "Numer bloku" },
    { key: "block_type", label: "Typ bloku" },
    { key: "trial_type", label: "Typ próby" },
    { key: "word_side", label: "Strona słowa" },
    { key: "LVF_stimulus", label: "Bodziec LVF" },
    { key: "RVF_stimulus", label: "Bodziec RVF" },
    { key: "correct_response", label: "Poprawna odpowiedź" },
    { key: "participant_response", label: "Odpowiedź uczestnika" },
    { key: "accuracy", label: "Poprawność" },
    { key: "reaction_time_ms", label: "RT (ms)" },
    { key: "response_time_from_stimulus", label: "Czas od bodźca (ms)" },
    { key: "no_response", label: "Brak odpowiedzi" },
    { key: "date", label: "Data" },
    { key: "time", label: "Godzina" },
    { key: "refresh_rate", label: "Odświeżanie monitora" },
    { key: "monitor_resolution", label: "Rozdzielczość monitora" },
    { key: "fullscreen", label: "Pełny ekran" },
    { key: "viewing_distance_cm", label: "Odległość od monitora (cm)" },
    { key: "fixation_duration", label: "Czas fiksacji (ms)" },
    { key: "stimulus_duration", label: "Czas bodźców (ms)" },
    { key: "response_window", label: "Okno odpowiedzi (ms)" },
    { key: "experiment_version", label: "Wersja eksperymentu" },
    { key: "random_seed", label: "Seed randomizacji" },
  ];
  const INSTRUCTION_PAGES = [
    ...
    {
      title: "Ważne",
      body: "Patrz cały czas na środek ekranu.\n\nBodźce są widoczne bardzo krótko, dlatego reaguj możliwie szybko i dokładnie.",
    },
    {
      title: "Trening",
      body: "Najpierw pojawi się 10 prób treningowych.\n\nPo treningu rozpocznie się część główna z 256 próbami.",
    },
  ];
  const SCREEN_IDS = [
  ...
  const ELEMENT_IDS = [
    ...
    "nav-calibration", "nav-editor", "nav-info", "nav-self-test", "home-backup-status", "home-download-backup",
    ...
    "calibration-monitor-resolution", "calibration-preview", "debug-show-hud", "debug-show-feedback",
    ...
    "summary-download", "summary-download-xlsx", "summary-download-log", "summary-restart", "error-body",
    "error-download-log", "error-download-csv", "error-download-xlsx", "error-restart", "self-test-output",
    "self-test-assets", "self-test-trials", "self-test-geometry", "self-test-psychojs",
    "self-test-csv", "self-test-e2e", "self-test-run", "self-test-select-all", "self-test-clear",
  ...
  async function init() {
    state.debugClockZero = performance.now();
    bindElements();
    exposeDebugApi();
    ...
    if (params.get("self_test") === "1") {
      ...
      renderSelfTestIdle();
      void runSelectedSelfTests(["assets", "trials", "geometry", "psychojs", "csv"]);
      return;
    }
  ...
  function exposeDebugApi() {
    window.__TLDT_WEB_DEBUG__ = {
      getStateSnapshot,
      getEvents: () => state.debugEvents.slice(),
      clearEvents: () => { state.debugEvents = []; },
    };
  }
  ...
  function getStateSnapshot() {
    return {
      phase: state.phase,
      readyMode: state.readyMode,
      paused: state.paused,
      awaitingResponse: state.awaitingResponse,
      currentIndex: state.currentIndex,
      currentTrial: state.currentTrial ? {
        trialNumber: state.currentTrial.trialNumber,
        trialType: state.currentTrial.trialType,
        lvfStimulus: state.currentTrial.lvfStimulus,
        rvfStimulus: state.currentTrial.rvfStimulus,
      } : null,
      currentTiming: state.currentTiming ? { ...state.currentTiming } : null,
      psychoVisibility: { ...state.psycho.lastVisibility },
      fullscreen: !!document.fullscreenElement,
      screen: SCREEN_IDS.find((id) => isScreenVisible(id)) || "",
    };
  }

  function debugRecord(name, details) {
    state.debugEvents.push({
      t_ms: round3(performance.now() - state.debugClockZero),
      name,
      details: details || {},
    });
    if (state.debugEvents.length > 6000) {
      state.debugEvents = state.debugEvents.slice(-6000);
    }
  }
  ...
  function bindEvents() {
    ...
    elements["home-download-backup"].addEventListener("click", downloadStoredBackupFromMenu);
    ...
    elements["debug-show-hud"].addEventListener("change", syncDebugOptionsFromUi);
    elements["debug-show-feedback"].addEventListener("change", syncDebugOptionsFromUi);
    ...
    bindClickIfPresent("instruction-next", advanceInstruction);
    bindClickIfPresent("ready-next", advanceReady);
    bindClickIfPresent("break-next", continueAfterBreak);
    ...
    elements["summary-download-xlsx"].addEventListener("click", downloadXlsxFallback);
    ...
    elements["error-download-xlsx"].addEventListener("click", downloadXlsxFallback);
    elements["error-restart"].addEventListener("click", resetToMenu);
    elements["self-test-run"].addEventListener("click", () => { void runSelectedSelfTests(); });
    elements["self-test-select-all"].addEventListener("click", () => setSelfTestCheckboxes(true));
    elements["self-test-clear"].addEventListener("click", () => setSelfTestCheckboxes(false));
  ...
  function loadStoredState() {
    ...
    if (storedCalibration) {
      ...
      if (typeof storedCalibration.debugShowHud === "boolean") {
        state.calibration.debugShowHud = storedCalibration.debugShowHud;
      }
      if (typeof storedCalibration.debugShowFeedback === "boolean") {
        state.calibration.debugShowFeedback = storedCalibration.debugShowFeedback;
      }
    }
  ...
  async function measureRefreshRate() {
    return new Promise((resolve, reject) => {
      ...
      const timeoutId = window.setTimeout(() => {
        ...
        settled = true;
        resolve(DEFAULT_MACBOOK_AIR_15_M4_PROFILE.refreshRate);
      }, 2000);
      function step(ts) {
        if (settled) {
          return;
  ...
  function refreshFormsFromState() {
    ...
    elements["debug-show-hud"].checked = !!state.calibration.debugShowHud;
    elements["debug-show-feedback"].checked = !!state.calibration.debugShowFeedback;
  ...
  function hydrateDefaultFileName() {
    ...
    const stamp = `${today.getFullYear()}${String(today.getMonth() + 1).padStart(2, "0")}${String(today.getDate()).padStart(2, "0")}_${String(today.getHours()).padStart(2, "0")}${String(today.getMinutes()).padStart(2, "0")}${String(today.getSeconds()).padStart(2, "0")}`;
  ...
  function syncDebugOptionsFromUi() {
    state.calibration.debugShowHud = !!elements["debug-show-hud"].checked;
    state.calibration.debugShowFeedback = !!elements["debug-show-feedback"].checked;
    applyDebugVisibility();
  }
  ...
  function deriveWordSide(correctResponse) {
    if (correctResponse === "f") return "left";
    if (correctResponse === "j") return "right";
    return "none";
  }

  function getPracticeTrialType(correctResponse) {
    if (correctResponse === "f") return "word_nonword";
    if (correctResponse === "j") return "nonword_word";
    return "nonword_nonword_ab";
  }

  function computePairFamilyKey(leftStimulus, rightStimulus) {
    return [String(leftStimulus || ""), String(rightStimulus || "")].sort().join("|");
  }

  function buildTrialTemplate(base) {
    return {
      ...base,
      pairFamilyKey: computePairFamilyKey(base.lvfStimulus, base.rvfStimulus),
    };
  }

  function pickBalancedShuffledBlock(trials, rng, recentHistoryKeys) {
    const history = Array.isArray(recentHistoryKeys) ? recentHistoryKeys.slice(-2) : [];
    const original = trials.map((trial) => ({ ...trial }));
    for (let attempt = 0; attempt < 400; attempt += 1) {
      const remaining = shuffleCopy(original, rng);
      const result = [];
      const recentKeys = history.slice();
      while (remaining.length > 0) {
        const blocked = recentKeys.slice(-2);
        const candidates = remaining
          .map((trial, index) => ({ trial, index }))
          .filter(({ trial }) => !blocked.includes(trial.pairFamilyKey));
        if (!candidates.length) {
          break;
        }
        const selected = candidates[Math.floor(rng() * candidates.length)];
        result.push(selected.trial);
        remaining.splice(selected.index, 1);
        recentKeys.push(selected.trial.pairFamilyKey);
      }
      if (result.length === original.length) {
        return result;
      }
    }
    throw new Error("Nie udało się ułożyć zrandomizowanego bloku bez powtórzeń tej samej pary w dwóch poprzednich próbach.");
  }

  function buildPracticeTrials(seed) {
    const rng = mulberry32(seed);
    const templates = state.stimuli.practice.map((trial, index) => buildTrialTemplate({
      phase: "practice",
      trialId: `practice_${String(index + 1).padStart(2, "0")}`,
      blockNumber: 0,
      blockType: "practice",
      trialType: getPracticeTrialType(trial.correctResponse),
      wordSide: deriveWordSide(trial.correctResponse),
      lvfStimulus: trial.lvf,
      rvfStimulus: trial.rvf,
      correctResponse: trial.correctResponse,
    }));
    return shuffleCopy(templates, rng).map((trial, index) => ({
      ...trial,
      trialNumber: index + 1,
    }));
  }
  ...
  function buildMainTrials(leftKey, rightKey, seed) {
    const uniqueTrials = [];
    state.stimuli.wordPairs.forEach(([word, nonword], index) => {
      const pairId = String(index + 1).padStart(2, "0");
      uniqueTrials.push(buildTrialTemplate({
        phase: "main",
        trialId: `wn_${pairId}`,
        blockType: "balanced_mixed",
    ...
    state.stimuli.nonwordPairs.forEach(([a, b], index) => {
      const pairId = String(index + 1).padStart(2, "0");
      uniqueTrials.push(buildTrialTemplate({
        phase: "main",
        trialId: `nn_ab_${pairId}`,
        blockType: "balanced_mixed",
        trialType: "nonword_nonword_ab",
        wordSide: "none",
        lvfStimulus: a,
        rvfStimulus: b,
      ...
      uniqueTrials.push(buildTrialTemplate({
        phase: "main",
        trialId: `nn_ba_${pairId}`,
        blockType: "balanced_mixed",
        trialType: "nonword_nonword_ba",
        wordSide: "none",
        lvfStimulus: b,
        rvfStimulus: a,
        correctResponse: "space",
    ...
    uniqueTrials.forEach((trial) => {
      blockOnePool.push({ ...trial, repetitionIndex: 1 });
      blockOnePool.push({ ...trial, repetitionIndex: 2 });
      blockTwoPool.push({ ...trial, repetitionIndex: 3 });
      blockTwoPool.push({ ...trial, repetitionIndex: 4 });
    });
    const blockOne = pickBalancedShuffledBlock(blockOnePool, rng, []);
    const blockTwo = pickBalancedShuffledBlock(
      blockTwoPool,
      rng,
      blockOne.slice(-2).map((trial) => trial.pairFamilyKey)
    );
    return blockOne.concat(blockTwo).map((trial, index) => ({
      ...trial,
      trialNumber: index + 1,
      blockNumber: index + 1 <= 128 ? 1 : 2,
    }));
  }

  function renderInstructionPage() {
    const page = INSTRUCTION_PAGES[state.instructionIndex];
    elements["instruction-progress"].textContent = "";
    elements["instruction-title"].textContent = page.title;
    elements["instruction-body"].innerHTML = page.body;
    setCenteredScreenFooter("instruction", "Naciśnij <strong>Spację</strong>, aby przejść dalej.");
    focusCenteredScreen("screen-instructions");
  ...
  function advanceInstruction() {
    ...
    state.phase = "instruction-transition";
  ...
  function showReadyScreen(mode) {
    ...
    if (mode === "practice") {
      ...
      elements["ready-body"].textContent =
        "Trening zawiera 10 prób. Po jego zakończeniu od razu rozpocznie się właściwa część badania.";
    } else {
      elements["ready-progress"].textContent = "";
      elements["ready-title"].textContent = "Za chwilę rozpocznie się część główna";
      elements["ready-body"].textContent =
        "Część główna zawiera 256 prób w dwóch blokach po 128. W połowie pojawi się krótka przerwa.";
    }
    showScreen("screen-ready");
    setCenteredScreenFooter("ready", "Naciśnij <strong>Spację</strong>, aby rozpocząć.");
    focusCenteredScreen("screen-ready");
  ...
  function resizePsychoJsStage() {
    ...
    if (state.awaitingResponse && state.currentTiming && !state.paused) {
      ...
      const showFixation = now < state.currentTiming.fixationEndsAt;
      const showStimulus = now >= state.currentTiming.stimulusOnsetAt && now < state.currentTiming.stimulusEndsAt;
  ...
  function setAutoDrawIfNeeded(stimulus, shouldDraw, cacheKey) {
    ...
    if (state.psycho.lastVisibility[cacheKey] !== shouldDraw) {
      ...
      debugRecord("visibility_change", { target: cacheKey, visible: shouldDraw });
    }
  }
  ...
  function renderPsychoJsFrame(showFixation, showLeft, showRight) {
    ...
    try {
      state.psycho.window._renderer.render(state.psycho.window._container, null, true);
    } catch (_error) {
  ...
  function runNextTrial() {
    ...
    if (state.phase === "practice") {
      ...
      debugRecord("trial_prepare", { phase: "practice", trialNumber: state.currentTrial.trialNumber });
    ...
    if (state.phase === "main") {
      ...
      state.currentTrial = state.currentList[state.currentIndex];
      debugRecord("trial_prepare", { phase: "main", trialNumber: state.currentTrial.trialNumber });
  ...
  function applyDebugVisibility() {
    toggle(elements["trial-phase-label"], !!state.calibration.debugShowHud);
    toggle(elements["trial-counter"], !!state.calibration.debugShowHud);
    if (!state.calibration.debugShowFeedback) {
  ...
  function startTrialAnimation() {
    ...
    const fixationSeconds = config.fixationMs / 1000;
    const stimulusSeconds = config.stimulusMs / 1000;
    const responseSeconds = config.responseMs / 1000;
    state.currentTiming = {
      fixationEndsAt: startAt + fixationSeconds,
      stimulusOnsetAt: startAt + fixationSeconds,
      stimulusEndsAt: startAt + fixationSeconds + stimulusSeconds,
      responseEndsAt: startAt + fixationSeconds + stimulusSeconds + responseSeconds,
    };
    state.awaitingResponse = true;
    state.paused = false;
    state.autoPilotResponseInjected = false;
    debugRecord("trial_start", {
      phase: state.phase,
      trialNumber: state.currentTrial ? state.currentTrial.trialNumber : null,
      fixationMs: config.fixationMs,
      stimulusMs: config.stimulusMs,
      responseMs: config.responseMs,
    });
    ...
    function tick() {
      ...
      if (state.paused) {
        ...
        renderPsychoJsFrame(showFixation, showStimulus, showStimulus);
        if (now >= timing.stimulusEndsAt) {
          injectAutoPilotResponse(timing);
            const capturedKeys = psychoJS.event
              .getKeys({ keyList: ["f", "j", "space"], timeStamped: true })
              .filter((entry) => Array.isArray(entry) && Number(entry[1]) >= timing.stimulusEndsAt);
  ...
  async function completeTrial(pressedKey, responseTimeFromStimulus, noResponse) {
    state.awaitingResponse = false;
    debugRecord("trial_complete", {
      phase: state.phase,
      trialNumber: state.currentTrial ? state.currentTrial.trialNumber : null,
      response: pressedKey || "",
      responseTimeFromStimulus: responseTimeFromStimulus == null ? null : formatMs(responseTimeFromStimulus),
      noResponse,
    });
    hide(elements["fixation"]);
    hide(elements["stimulus-left"]);
    ...
    if (state.phase === "practice" && state.calibration.debugShowFeedback) {
  ...
  async function showPracticeSummary() {
    ...
    elements["break-progress"].textContent = "";
    elements["break-title"].textContent = "Trening zakończony";
    elements["break-body"].textContent =
      "Za chwilę rozpocznie się część główna badania. Zachowaj wzrok na środku ekranu i przygotuj się do kolejnych prób.";
    showScreen("screen-break");
    setCenteredScreenFooter("break", "Naciśnij <strong>Spację</strong>, aby rozpocząć część główną.");
    focusCenteredScreen("screen-break");
  ...
  async function showMidwayBreak() {
    ...
    elements["break-body"].textContent =
      "Ukończono połowę badania.\n\nNaciśnij Spację, aby kontynuować.";
    showScreen("screen-break");
    setCenteredScreenFooter("break", "Naciśnij <strong>Spację</strong>, aby kontynuować.");
    focusCenteredScreen("screen-break");
    maybeAutoAdvanceBreak();
  }

  async function continueAfterBreak() {
    if (!(state.phase === "practice-summary" || state.phase === "main-break")) {
      return;
    }
    const currentPhase = state.phase;
    state.phase = "break-transition";
    if (currentPhase === "practice-summary") {
      await startMain();
      return;
    }
  ...
  function focusPrimaryAction(element) {
    window.setTimeout(() => {
      try {
        window.focus();
  ...
  function focusCenteredScreen(screenId) {
    window.setTimeout(() => {
      try {
        window.focus();
  ...
  function setCenteredScreenFooter(kind, html) {
    const selectors = {
      instruction: "#screen-instructions .instruction-footer",
      ready: "#screen-ready .instruction-footer",
      break: "#screen-break .instruction-footer",
    };
    const node = document.querySelector(selectors[kind]);
    if (node) {
      node.innerHTML = html;
    }
  }
  ...
  async function finishExperiment(status) {
    ...
    buildCsvDownloadBlob();
    buildXlsxDownloadBlob();
    buildLogBlob();
    persistBackup();
    const mainRows = state.rows.filter((row) => row.phase === "main");
    const saveInfo = state.fileHandle
      ? `Dane były dopisywane do wybranego pliku CSV: ${state.fileHandle.name}.`
      : "Nie wybrano pliku bezpośredniego zapisu. Dane są dostępne do pobrania jako CSV lub XLSX.";
    elements["summary-status"].textContent = status === "completed" ? "Koniec badania" : "Sesja została przerwana";
    elements["summary-title"].textContent = status === "completed" ? "Dziękujemy za udział w badaniu" : "Sesja została przerwana";
    elements["summary-body"].textContent =
      `${status === "completed" ? "Badanie zostało zakończone poprawnie." : "Badanie zostało przerwane przed końcem."}\n\n` +
      ...
      "Na tym ekranie możesz pobrać wyniki jako CSV lub XLSX.";
    showScreen("screen-summary");
    focusPrimaryAction(elements["summary-download"]);
    if (state.selfTestMode && state.autoPilotEnabled) {
      finalizeAutoPilotE2E(status, [], mainRows);
    }
  }
  ...
  function updateSaveStatus(message) {
    elements["save-status"].textContent = message || (state.fileHandle
      ...
      : "Jeśli chcesz wskazać dokładną lokalizację CSV, kliknij przycisk wyboru pliku. Domyślna nazwa korzysta z ID uczestnika i czasu uruchomienia, więc każdy badany może mieć osobny plik. Niezależnie od tego aplikacja zapisuje kopię awaryjną w pamięci przeglądarki.");
  }
  ...
  function buildXlsxWorksheet(dataMatrix) {
    const worksheet = {};
    const rowCount = dataMatrix.length;
    const columnCount = dataMatrix.reduce((max, row) => Math.max(max, row.length), 0);
    for (let rowIndex = 0; rowIndex < rowCount; rowIndex += 1) {
      for (let columnIndex = 0; columnIndex < columnCount; columnIndex += 1) {
        const value = dataMatrix[rowIndex][columnIndex] == null ? "" : dataMatrix[rowIndex][columnIndex];
        const cellRef = XLSX.utils.encode_cell({ r: rowIndex, c: columnIndex });
        if (typeof value === "number" && Number.isFinite(value)) {
          worksheet[cellRef] = { v: value, t: "n" };
        } else if (typeof value === "boolean") {
          worksheet[cellRef] = { v: value, t: "b" };
        } else {
    ...
    worksheet["!ref"] = XLSX.utils.encode_range({
      s: { r: 0, c: 0 },
      e: { r: Math.max(0, rowCount - 1), c: Math.max(0, columnCount - 1) },
    });
    return worksheet;
  }
  ...
  function buildXlsxDownloadBlob() {
    if (!window.XLSX || !XLSX.utils || typeof XLSX.write !== "function") {
      throw new Error("Brakuje biblioteki XLSX potrzebnej do eksportu arkusza.");
    }
    if (state.lastXlsxBlobUrl) {
      URL.revokeObjectURL(state.lastXlsxBlobUrl);
    }
    const data = [XLSX_COLUMNS.map((column) => column.label)];
    state.rows.forEach((row) => {
      data.push(XLSX_COLUMNS.map((column) => row[column.key] == null ? "" : row[column.key]));
    });
    const worksheet = buildXlsxWorksheet(data);
    worksheet["!autofilter"] = {
      ref: XLSX.utils.encode_range({
        s: { r: 0, c: 0 },
        e: { r: Math.max(0, data.length - 1), c: Math.max(0, XLSX_COLUMNS.length - 1) },
      }),
    };
    worksheet["!freeze"] = { xSplit: 0, ySplit: 1, topLeftCell: "A2", activePane: "bottomLeft", state: "frozen" };
    worksheet["!cols"] = XLSX_COLUMNS.map((column, index) => {
      const maxWidth = data.reduce((width, row) => Math.max(width, String(row[index] == null ? "" : row[index]).length), column.label.length);
      return { wch: Math.min(Math.max(maxWidth + 2, 12), 36) };
    });
    const workbook = {
      SheetNames: ["Wyniki"],
      Sheets: { Wyniki: worksheet },
    };
    const binaryString = XLSX.write(workbook, { bookType: "xlsx", type: "binary" });
    const arrayBuffer = binaryStringToArrayBuffer(binaryString);
    state.lastXlsxBlobUrl = URL.createObjectURL(
      new Blob(
        [arrayBuffer],
  ...
  function downloadXlsxFallback() {
    ...
    try {
      ...
      const anchor = document.createElement("a");
      anchor.href = state.lastXlsxBlobUrl;
      anchor.download = (state.config ? state.config.csvName : "wyniki.csv").replace(/\.csv$/i, ".xlsx");
      document.body.appendChild(anchor);
      anchor.click();
      anchor.remove();
    } catch (error) {
  ...
  function downloadStoredBackupFromMenu() {
    const backup = safeParseJson(loadStorage(STORAGE_KEYS.backup));
    if (!backup || !backup.csvText) {
      handleNonFatalMessage("Nie znaleziono zapisanej kopii awaryjnej CSV w pamięci przeglądarki.");
      return;
    }
    const blob = new Blob([backup.csvText], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = backup.csvName || (backup.meta && backup.meta.sessionId ? `${backup.meta.sessionId}.csv` : "kopia_awaryjna.csv");
    document.body.appendChild(anchor);
    anchor.click();
    anchor.remove();
  ...
  function updateBackupNotice() {
    const backup = safeParseJson(loadStorage(STORAGE_KEYS.backup));
    const hasBackup = !!(backup && backup.meta && backup.csvText);
    elements["home-backup-status"].textContent = !hasBackup
      ? "Kopia awaryjna wyników będzie zapisywana w pamięci przeglądarki po każdej próbie."
      : `Ostatnia kopia awaryjna: ${backup.meta.sessionId}. Zapis ostatniej zmiany: ${backup.updatedAt || "brak danych"}. Jeśli zapomniano pobrać wynik po badaniu, użyj przycisku „Pobierz ostatnią kopię CSV”.`;
    if (elements["home-download-backup"]) {
      elements["home-download-backup"].disabled = !hasBackup;
    }
  }
  ...
  function handleFatalError(error) {
    ...
    try {
      ...
      // Ignore XLSX export preparation errors during fatal handling.
    }
    buildLogBlob();
    persistBackup();
  ...
  function onKeyDown(event) {
    ...
    if (isScreenVisible("screen-instructions") && code === "Space") {
      event.preventDefault();
      debugRecord("space_advance", { screen: "instructions" });
      advanceInstruction();
      return;
    }
    if (isScreenVisible("screen-ready") && code === "Space") {
      event.preventDefault();
      debugRecord("space_advance", { screen: "ready", mode: state.readyMode });
      advanceReady();
      return;
    }
    if ((state.phase === "practice-summary" || state.phase === "main-break") && code === "Space") {
      event.preventDefault();
      debugRecord("space_advance", { screen: "break", phase: state.phase });
      continueAfterBreak();
      return;
    }
    ...
    if (state.psycho.enabled) {
      ...
      if (now < state.currentTiming.stimulusEndsAt) {
        return;
      }
      event.preventDefault();
  ...
  function handleFullscreenChange() {
    ...
    const fullscreenManagedScreen = isScreenVisible("screen-instructions") ||
      ...
      isScreenVisible("screen-summary");
  ...
  function normalizeCsvName(value, participantId) {
    ...
    const source = (value || "").trim() || `${participantId}_${fallbackStamp}.csv`;
  ...
  function buildCsvValidationReport(rows) {
    ...
    const trialIdBlockCounts = new Map();
    const repeatViolations = [];
    mainRows.forEach((row, index) => {
      counts.set(row.trial_id, (counts.get(row.trial_id) || 0) + 1);
      const blockKey = `${row.block_number}|${row.trial_type}`;
      byBlockType.set(blockKey, (byBlockType.get(blockKey) || 0) + 1);
      const trialBlockKey = `${row.block_number}|${row.trial_id}`;
      trialIdBlockCounts.set(trialBlockKey, (trialIdBlockCounts.get(trialBlockKey) || 0) + 1);
      const currentPairKey = computePairFamilyKey(row.LVF_stimulus, row.RVF_stimulus);
      const previousRows = mainRows.slice(Math.max(0, index - 2), index);
      if (previousRows.some((previousRow) => computePairFamilyKey(previousRow.LVF_stimulus, previousRow.RVF_stimulus) === currentPairKey)) {
        repeatViolations.push({ trial_number: row.trial_number, trial_id: row.trial_id });
      }
    });
    return {
      total: rows.length,
      ...
      unique: counts.size,
      allRepeated4: [...counts.values()].every((value) => value === 4),
      balancedByBlock: [1, 2].every((blockNumber) => (
        (byBlockType.get(`${blockNumber}|word_nonword`) || 0) === 32 &&
        (byBlockType.get(`${blockNumber}|nonword_word`) || 0) === 32 &&
        (byBlockType.get(`${blockNumber}|nonword_nonword_ab`) || 0) === 32 &&
        (byBlockType.get(`${blockNumber}|nonword_nonword_ba`) || 0) === 32
      )),
      allTrialIdsRepeated2PerBlock: [...trialIdBlockCounts.values()].every((value) => value === 2),
      repeatViolations,
    };
  }
  ...
  function runSelfTests() {
    return runSelectedSelfTests(["assets", "trials", "geometry", "psychojs", "csv"]);
  }

  function renderSelfTestIdle() {
    if (!elements["self-test-output"]) {
      return;
    }
    elements["self-test-output"].textContent =
      "Wybierz testy i nacisnij Start testow aplikacji.\n" +
      "Raport pojawi sie tutaj po zakonczeniu walidacji.";
  }

  function setSelfTestCheckboxes(checked) {
    ["self-test-assets", "self-test-trials", "self-test-geometry", "self-test-psychojs", "self-test-csv", "self-test-e2e"]
      .forEach((id) => {
        elements[id].checked = checked;
      });
  }

  function appendSelfTestOutput(line) {
    const current = elements["self-test-output"].textContent || "";
    elements["self-test-output"].textContent = current ? `${current}\n${line}` : line;
  }

  function getSelectedSelfTests(explicitSelection) {
    if (Array.isArray(explicitSelection) && explicitSelection.length) {
      return explicitSelection.slice();
    }
    const mapping = [
      ["self-test-assets", "assets"],
      ["self-test-trials", "trials"],
      ["self-test-geometry", "geometry"],
      ["self-test-psychojs", "psychojs"],
      ["self-test-csv", "csv"],
      ["self-test-e2e", "e2e"],
    ];
    return mapping.filter(([id]) => elements[id] && elements[id].checked).map(([, value]) => value);
  }

  async function runSelectedSelfTests(explicitSelection) {
    if (state.selfTestRunning) {
      return;
    }
    const selected = getSelectedSelfTests(explicitSelection);
    showScreen("screen-self-test");
    if (!selected.length) {
      elements["self-test-output"].textContent = "Nie zaznaczono ?adnego testu.";
      return;
    }
      state.selfTestRunning = true;
      elements["self-test-run"].disabled = true;
      elements["self-test-select-all"].disabled = true;
      elements["self-test-clear"].disabled = true;
      elements["self-test-output"].textContent = `Start walidacji. Wybrano ${selected.length} test(y).\n`;

    const tests = {
      assets: { label: "Biblioteki PsychoJS i zaleznosci", run: runSelfTestAssets },
      trials: { label: "Generator prob i kombinacje 64 x 4", run: runSelfTestTrials },
      geometry: { label: "Geometria, parametry srodowiska i format czasu", run: runSelfTestGeometry },
      psychojs: { label: "Renderowanie PsychoJS i obsluga pelnego ekranu", run: runSelfTestPsychoJs },
      csv: { label: "Kopia awaryjna i struktura CSV", run: runSelfTestCsv },
      e2e: { label: "Pelny test end-to-end 10 + 256", run: runSelfTestE2E },
    };
    ...
    try {
      for (const key of selected) {
        ...
        appendSelfTestOutput(`START: ${test.label}...`);
        try {
          const detail = await test.run();
          results.push({ key, ok: true, detail });
          appendSelfTestOutput(`OK  ${test.label}${detail ? ` -> ${detail}` : ""}`);
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error);
          results.push({ key, ok: false, detail: message });
          appendSelfTestOutput(`BLAD  ${test.label} -> ${message}`);
        }
      }
      const okCount = results.filter((result) => result.ok).length;
      const failCount = results.length - okCount;
      appendSelfTestOutput("");
      appendSelfTestOutput(`Wynik koncowy: ${okCount}/${results.length} testow zaliczonych.`);
      if (failCount === 0) {
        document.title = "WEB_PSYCHOJS_SELF_TEST_OK";
        appendSelfTestOutput("Status: OK");
      } else {
        document.title = "WEB_PSYCHOJS_SELF_TEST_FAIL";
        appendSelfTestOutput(`Status: BLAD (${failCount})`);
      }
    } finally {
      state.selfTestRunning = false;
      elements["self-test-run"].disabled = false;
      elements["self-test-select-all"].disabled = false;
      elements["self-test-clear"].disabled = false;
    }
  }

  function runSelfTestAssets() {
    assert(psychoJsAssetsAvailable(), "Biblioteki PsychoJS nie zostaly zaladowane.");
    const parsed = validateStimuliPayload(state.stimuli);
    assert(parsed.ok, parsed.message || "Walidacja bodzcow nie powiodla sie.");
    return "biblioteki i bodzce sa dostepne";
  }

  function runSelfTestTrials() {
    [1001, 1002, 1003].forEach((seed) => validateTrialSet(buildMainTrials("f", "j", seed)));
    const practiceTrials = buildPracticeTrials(1004);
    assert(practiceTrials.length === 10, "Trening powinien miec 10 prob.");
    assert(practiceTrials.every((trial, index) => trial.trialNumber === index + 1), "W treningu trial_number powinien oznaczac kolejnosc prezentacji.");
    return "3 rozne seedy, zbalansowane bloki i trial_number przeszly walidacje";
  }

  function runSelfTestGeometry() {
    const geometry = computeGeometry(DEFAULT_MACBOOK_AIR_15_M4_PROFILE.monitorWidthCm, 2880);
    assert(geometry.fontPx > 0 && geometry.offsetPx > 0, "Geometria musi dawac dodatnie wartosci.");
    const timingCheck = { fixationMs: 1000, stimulusMs: 100, responseMs: 2000 };
    const totalWindowMs = timingCheck.fixationMs + timingCheck.stimulusMs + timingCheck.responseMs;
    assert(totalWindowMs === 3100, "Pojedyncza proba powinna miec uklad 1000 + 100 + 2000 ms.");
    const clockString = formatClockTimeWithMs(new Date("2026-04-08T13:14:15.321"));
    assert(/^\d{2}:\d{2}:\d{2}\.\d{3}$/.test(clockString), "Czas klawiatury i CSV powinien zawierac milisekundy.");
    assert(Boolean(elements["refresh-rate-hz"].value.trim()), "Pole refresh rate nie powinno byc puste.");
    assert(/^\d{3,5}x\d{3,5}$/i.test(elements["monitor-resolution"].value.trim()), "Pole rozdzielczosci powinno miec format szerokoscxwysokosc.");
    state.phase = "instructions";
    state.instructionIndex = 0;
    renderInstructionPage();
    showScreen("screen-instructions");
    assert(window.getComputedStyle(document.body).backgroundColor === "rgb(255, 255, 255)", "Ekrany instrukcji powinny miec biale tlo.");
    assert(window.getComputedStyle(document.querySelector("#screen-instructions .instruction-card")).backgroundColor === "rgb(255, 255, 255)", "Karta instrukcji powinna miec biale tlo.");
    assert(document.getElementById("instruction-next") === null, "Przycisk Dalej nie powinien byc obecny na ekranie instrukcji.");
    assert(document.getElementById("ready-next") === null, "Przycisk Rozpocznij nie powinien byc obecny na ekranie gotowosci.");
    assert(document.getElementById("break-next") === null, "Przycisk Kontynuuj nie powinien byc obecny na ekranie przerwy.");
    assert(window.getComputedStyle(document.querySelector("#screen-instructions .instruction-card")).textAlign === "center", "Tekst instrukcji powinien byc wysrodkowany.");
    showScreen("screen-self-test");
    return "geometria, pola konfiguracji i format czasu sa poprawne";
  }

  function runSelfTestPsychoJs() {
    ...
    assert(state.paused === true, "Zmiana fullscreen powinna chwilowo zatrzymac probe na czas przeliczenia layoutu.");
    state.awaitingResponse = false;
    state.currentTiming = null;
    state.paused = false;
    state.fullscreenTransitionActive = false;
    ...
    hide(elements["fullscreen-overlay"]);
    showScreen("screen-self-test");
    return "render i reakcja na fullscreen dzialaja";
  }

  function runSelfTestCsv() {
    const syntheticRows = createSyntheticRowsForTest();
    const practiceRows = syntheticRows.filter((row) => row.phase === "practice");
    const practiceStats = buildResponseStatsSummary(practiceRows);
    assert(practiceStats.includes("Lewa strona (F):") && practiceStats.includes("%"), "Podsumowanie treningu powinno pokazywac procenty.");
    const validation = buildCsvValidationReport(syntheticRows);
    assert(validation.mainRows === 256, "CSV syntetyczny powinien miec 256 prob glownych.");
    assert(validation.unique === 64, "CSV syntetyczny powinien miec 64 kombinacje.");
    assert(validation.allRepeated4, "Kazda kombinacja musi wystapic 4 razy.");
    assert(validation.balancedByBlock, "Oba bloki powinny byc zbalansowane po 32 proby kazdego typu.");
    assert(validation.allTrialIdsRepeated2PerBlock, "Kazdy trial_id powinien wystapic dwa razy w kazdym bloku.");
    assert(validation.repeatViolations.length === 0, "Nie powinno byc powtorzen tej samej pary w dwoch poprzednich probach.");
    assert(syntheticRows.length === 266, "Calosc powinna miec 266 wierszy razem z treningiem.");
    saveStorage(STORAGE_KEYS.backup, JSON.stringify({ meta: { sessionId: "TEST_1" }, rows: syntheticRows }));
    assert(loadStorage(STORAGE_KEYS.backup).includes("TEST_1"), "Kopia awaryjna powinna sie zapisac.");
    assert(["phase", "trial_id", "block_type", "word_side"].every((field) => CSV_FIELDS.includes(field)), "CSV powinno zawierac dodatkowe pola analityczne.");
    const previousRows = state.rows;
    const previousCsvCache = state.csvTextCache;
    try {
      state.rows = syntheticRows.slice();
      state.csvTextCache = buildCsvText();
      buildXlsxDownloadBlob();
      assert(Boolean(state.lastXlsxBlobUrl), "Eksport XLSX powinien przygotowac plik do pobrania.");
    } finally {
      state.rows = previousRows;
      state.csvTextCache = previousCsvCache;
    }
    return "CSV, backup, pola analityczne i procentowe statystyki sa poprawne";
  }

  async function runSelfTestE2E() {
    state.selfTestMode = true;
    state.autoPilotEnabled = true;
    await runE2ETest();
    await waitForSelfTestTitle(["WEB_PSYCHOJS_E2E_TEST_OK", "WEB_PSYCHOJS_E2E_TEST_FAIL"], 90000);
    if (document.title !== "WEB_PSYCHOJS_E2E_TEST_OK") {
      throw new Error("Pelny test end-to-end nie zakonczyl sie powodzeniem.");
    }
    return "pelny przebieg 10 + 256 zakonczyl sie poprawnie";
  }

  function waitForSelfTestTitle(acceptedTitles, timeoutMs) {
    return new Promise((resolve, reject) => {
      const deadline = performance.now() + timeoutMs;
      function check() {
        if (acceptedTitles.includes(document.title)) {
          resolve(document.title);
          return;
        }
        if (performance.now() >= deadline) {
          reject(new Error("Przekroczono czas oczekiwania na wynik testu."));
          return;
        }
  ...
  function finalizeAutoPilotE2E(status, practiceRows, mainRows) {
    try {
      const effectivePracticeRows = practiceRows.length ? practiceRows : state.rows.filter((row) => row.phase === "practice");
      const effectiveMainRows = mainRows.length ? mainRows : state.rows.filter((row) => row.phase === "main");
      assert(status === "completed", "E2E: badanie nie zostało zakończone.");
      const validation = buildCsvValidationReport(state.rows);
      assert(effectivePracticeRows.length === 10, "E2E: trening powinien mieć 10 prób.");
      assert(effectiveMainRows.length === 256, "E2E: liczba prób głównych.");
      ...
      assert(validation.balancedByBlock, "E2E: oba bloki powinny byc zbalansowane.");
      assert(validation.allTrialIdsRepeated2PerBlock, "E2E: każdy trial_id powinien być dwa razy w każdym bloku.");
      assert(validation.repeatViolations.length === 0, "E2E: nie powinno być powtórzeń tej samej pary w dwóch poprzednich próbach.");
      assert(state.csvTextCache.split("\n").length === 268, "E2E: poprawna liczba linii CSV wraz z pustą końcówką.");
      assert(effectiveMainRows.every((row) => row.response_time_from_stimulus !== "" && Number(row.response_time_from_stimulus) > 0), "E2E: każde RT powinno być dodatnie.");
      elements["self-test-output"].textContent =
        "WEB_PSYCHOJS_E2E_TEST_OK\n" +
        "Przeszedł pełny rzeczywisty przebieg: 10 prób treningowych + 256 prób głównych.\n" +
        "Zweryfikowano podział 128/128, 64 kombinacje × 4, zbalansowane bloki, brak lokalnych powtórzeń par, zapis CSV i dodatnie czasy reakcji.\n" +
        `Średni czas RT w teście automatycznym: ${computeMeanRt(effectiveMainRows)} ms.`;
  ...
  function buildSyntheticRow(trial, pressedKey, responseTimeFromStimulus, noResponse) {
    return {
      participant_ID: "TEST", phase: trial.phase, trial_number: trial.trialNumber, trial_id: trial.trialId, block_number: trial.blockNumber, block_type: trial.blockType, trial_type: trial.trialType, word_side: trial.wordSide,
  ...
  function validateTrialSet(trials) {
    ...
    const counts = new Map();
    const byType = { word_nonword: 0, nonword_word: 0, nonword_nonword_ab: 0, nonword_nonword_ba: 0 };
    const byBlockType = new Map();
    const trialIdBlockCounts = new Map();
    trials.forEach((trial) => {
      ...
      const blockKey = `${trial.blockNumber}|${trial.trialType}`;
      byBlockType.set(blockKey, (byBlockType.get(blockKey) || 0) + 1);
      const trialBlockKey = `${trial.blockNumber}|${trial.trialId}`;
      trialIdBlockCounts.set(trialBlockKey, (trialIdBlockCounts.get(trialBlockKey) || 0) + 1);
    });
    ...
    assert(byType.nonword_nonword_ab === 64, "Typ nonword_nonword_ab powinien mieć 64 próby.");
    assert(byType.nonword_nonword_ba === 64, "Typ nonword_nonword_ba powinien mieć 64 próby.");
    [1, 2].forEach((blockNumber) => {
      assert((byBlockType.get(`${blockNumber}|word_nonword`) || 0) === 32, "Każdy blok powinien mieć 32 próby word_nonword.");
      assert((byBlockType.get(`${blockNumber}|nonword_word`) || 0) === 32, "Każdy blok powinien mieć 32 próby nonword_word.");
      assert((byBlockType.get(`${blockNumber}|nonword_nonword_ab`) || 0) === 32, "Każdy blok powinien mieć 32 próby nonword_nonword_ab.");
      assert((byBlockType.get(`${blockNumber}|nonword_nonword_ba`) || 0) === 32, "Każdy blok powinien mieć 32 próby nonword_nonword_ba.");
    });
    assert([...trialIdBlockCounts.values()].every((value) => value === 2), "Każdy trial_id powinien wystąpić dwa razy w każdym bloku.");
    trials.forEach((trial, index) => {
      const currentPairKey = trial.pairFamilyKey || computePairFamilyKey(trial.lvfStimulus, trial.rvfStimulus);
      const previousTrials = trials.slice(Math.max(0, index - 2), index);
      assert(!previousTrials.some((previousTrial) => {
        const previousPairKey = previousTrial.pairFamilyKey || computePairFamilyKey(previousTrial.lvfStimulus, previousTrial.rvfStimulus);
        return previousPairKey === currentPairKey;
      }), "Ta sama para bodźców nie powinna pojawić się w dwóch poprzednich próbach.");
    });
  }
